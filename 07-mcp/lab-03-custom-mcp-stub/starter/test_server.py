...
We invoke the server as a subprocess, send each JSON-RPC message on its own
line, then read one line back per message. This is exactly how Claude Code
talks to a stdio MCP server, minus the complexity of long-running sessions.
"""
...
def rpc(messages: list[dict]) -> list[dict]:
    """Run server.py, send each message, return the parsed responses."""
...
class TestToolsList(unittest.TestCase):
    def test_tools_list_has_get_quote_and_echo(self):
        ...
        # Each tool must have description + inputSchema.
        for tool in tools:
...
class TestToolsCall(unittest.TestCase):
    ...
    def test_get_quote_returns_non_empty_text(self):
        ...
        content = resp["result"]["content"]
        self.assertEqual(content[0]["type"], "text")
        self.assertTrue(content[0]["text"])  # non-empty
